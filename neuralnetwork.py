...
class NeuralNetwork():
	...
	def backpropagate(self, z_s, a_s, random_batch): 
		...
		deltas[-1] = (self.training_outputs[random_batch]-a_s[-1])*(sigmoid_derivative(z_s[-1]))
	...
	#Gradient descent
	def adjust(self, lr = .1, method ='batch', batch_size=20):
		if method == 'batch':
		#Pass training set through neural network
			...
			self.random_batch = random.sample(range(self.training_batch_size),batch_size)
	...
	#Multiple ajustments
	def train(self, epochs, lr=.1, method='minibatch', batch_size =20):
	...
	def training_error(self):
		return np.average((self.training_outputs-self.output(self.training_inputs))**2)
